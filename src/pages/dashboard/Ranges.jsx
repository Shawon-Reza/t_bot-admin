...
const emptyForm = { name: '', countryId: '', distributionCountPerRange: '', expirationTimeInMinutes: '', distributeNewNumberAfterMinutes: '' };
...
const Ranges = () => {
  ...
  const handleOpenModal = (range = null) => {
    ...
    setFormData(range ? { name: range.name, countryId: range.countryId, distributionCountPerRange: range.distributionCountPerRange, expirationTimeInMinutes: range.expirationTimeInMinutes, distributeNewNumberAfterMinutes: range.distributeNewNumberAfterMinutes } : emptyForm);
    setShowModal(true);
  };
  ...
  const handleSubmit = (event) => {
    ...
    if (editingRange) updateMutation.mutate({ id: editingRange.id, range: { ...formData, distributionCountPerRange: Number(formData.distributionCountPerRange), expirationTimeInMinutes: Number(formData.expirationTimeInMinutes), distributeNewNumberAfterMinutes: Number(formData.distributeNewNumberAfterMinutes) } });
    else createMutation.mutate({ name: formData.name, countryId: formData.countryId });
  };
...
+      </div>
  ...
  {showModal && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={handleCloseModal}><div className="bg-white rounded-2xl shadow-xl w-full max-w-md" onClick={(event) => event.stopPropagation()}><div className="p-5 border-b border-gray-100 flex items-center justify-between"><h2 className="text-lg font-semibold text-gray-900">{editingRange ? 'Edit Range' : 'Add New Range'}</h2><button onClick={handleCloseModal} className="p-2 rounded-lg hover:bg-gray-100"><FiX /></button></div><form onSubmit={handleSubmit} className="p-5 space-y-5"><div><label className="block text-sm font-medium text-gray-700 mb-2">Range Name</label><input value={formData.name} onChange={(event) => setFormData({ ...formData, name: event.target.value })} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-2">Country</label><select value={formData.countryId} onChange={(event) => setFormData({ ...formData, countryId: event.target.value })} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-white" required><option value="">Select Country</option>{countries.map((country) => <option key={country.id} value={country.id}>{country.name}</option>)}</select></div>{editingRange && <div className="grid gap-4 sm:grid-cols-3"><div><label className="block text-sm font-medium text-gray-700 mb-2">Numbers per distribution</label><input type="number" min="1" step="1" value={formData.distributionCountPerRange} onChange={(event) => setFormData({ ...formData, distributionCountPerRange: event.target.value })} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-2">OTP expiry (minutes)</label><input type="number" min="0.1" step="0.1" value={formData.expirationTimeInMinutes} onChange={(event) => setFormData({ ...formData, expirationTimeInMinutes: event.target.value })} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl" required /></div><div><label className="block text-sm font-medium text-gray-700 mb-2">Change cooldown (minutes)</label><input type="number" min="0" step="0.1" value={formData.distributeNewNumberAfterMinutes} onChange={(event) => setFormData({ ...formData, distributeNewNumberAfterMinutes: event.target.value })} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl" required /></div></div>}<div className="flex gap-3"><button type="button" onClick={handleCloseModal} className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl">Cancel</button><button type="submit" disabled={mutationPending} className="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-xl disabled:opacity-50">{mutationPending ? 'Saving...' : editingRange ? 'Update' : 'Create'}</button></div></form></div></div>}
